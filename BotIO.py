...
async def knopki(m: types.Message):
    text = m.text.split()[1:]
    if text[0] == passGen(m):
        await imo.summon(model, bot, 'settings', m, locked = True, pdata = ['Меню с настройками(кнопоки)', ['СВИТЧИ','bool:reboot:0', 'close:toggles:0'], ['FIT','counter:fit:50:0','send:fit:0','close:fit:0'], ['RESET','counter:neurod:1024:0','counter:layers:1:0','send:reset:0','close:reset:0'], 'close:null:0'])
